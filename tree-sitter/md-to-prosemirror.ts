...
function toProseMirrorJSON(node: any, source: string): any {
  ...
  switch (node.type) {
    ...
    case 'strong': {
      ...
      console.warn(
        `Unhandled strong node structure: ${JSON.stringify(content)}`,
      );
      return null;
    }
    ...
    case 'emphasis': {
      ...
      console.warn(
        `Unhandled emphasis node structure: ${JSON.stringify(content)}`,
      );
      return null;
    }
...
console.log(
  JSON.stringify(
    toProseMirrorJSON(currentTree.rootNode, currentSource),
    null,
    2,
  ),
);
...
currentSource = currentSource.slice(0, insertStartByte) + 'Updated ' +
  currentSource.slice(insertEndByte);
