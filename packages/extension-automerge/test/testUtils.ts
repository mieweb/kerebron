...
import { applyBlockPatch } from '../src/maintainSpans.ts';
import { isArrayEqual, isPrefixOfArray } from '../src/utils.ts';
  ...
  doc = automerge.change(doc, (doc) => {
    for (const line of notation) {
      if (typeof line === 'string') {
        ...
        const block = {
          type: new automerge.RawString(line.type),
          parents: line.parents.map((p) => new automerge.RawString(p)),
          attrs: line.attrs,
        };
...
export function makeDoc(
  ...
  const pmDoc = amSpansToDoc(adapter, spans);
  ...
  return (path: automerge.Prop[]): automerge.Patch[] => {
    const patches: automerge.Patch[] = [
      ...
      },
      {
        ...
        value: new automerge.RawString(block.type),
      },
      {
        action: 'put',
    ...
    block.parents.forEach((parent, i) => {
      patches.push({
        ...
        values: [new automerge.RawString(parent)],
      });
    });
...
export function assertSplitBlock(
  ...
  const expectedSpan: { [key: string]: automerge.MaterializeValue } = {
    type: new automerge.RawString(expected.type),
    parents: expected.parents.map((p) => new automerge.RawString(p)),
  ...
  assertEquals(block, expectedSpan);
}
