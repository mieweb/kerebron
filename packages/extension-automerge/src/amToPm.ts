...
import { amSpansToDoc, amSpliceIdxToPmIdx } from './amTraversal.ts';
import { findBlockAtCharIdx, patchSpans } from './maintainSpans.ts';
import { isArrayEqual, isPrefixOfArray } from './utils.ts';
import { pmMarksFromAmMarks, SchemaAdapter } from './SchemaAdapter.ts';
...
type MarkPatch = {
  action: 'mark';
  path: Prop[];
  marks: automerge.Mark[];
};

export function amToPm(
  adapter: SchemaAdapter,
  spansAtStart: automerge.Span[],
  patches: Array<Patch>,
  path: Prop[],
  tx: Transaction,
): Transaction {
...
type TextPatches = {
  type: 'text';
  patches:
    (automerge.SpliceTextPatch | automerge.DelPatch | automerge.MarkPatch)[];
};
...
function gatherPatches(
  textPath: automerge.Prop[],
  diff: automerge.Patch[],
): GatheredPatch[] {
  const result: GatheredPatch[] = [];

  type State =
    | { type: 'gatheringBlock'; index: number; gathered: automerge.Patch[] }
    | {
      type: 'gatheringText';
      gathered: (
        | automerge.SpliceTextPatch
        | automerge.DelPatch
        | automerge.MarkPatch
      )[];
    }
