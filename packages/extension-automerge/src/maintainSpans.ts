...
import { isArrayEqual, isPrefixOfArray } from './utils.ts';

export function patchSpans(
  atPath: automerge.Prop[],
...
function markSpans(spans: automerge.Span[], patch: automerge.Mark): void {
  let textPos = 0;
  let i = 0;
...
function deleteSpans(spans: automerge.Span[], patch: automerge.DelPatch): void {
...
export function applyBlockPatch(
  parentPath: automerge.Prop[],
  patch: automerge.Patch,
  block: { [key: string]: automerge.MaterializeValue },
) {
  ...
  if (patch.action === 'put') {
    ...
    const arr = target[prop] as automerge.MaterializeValue[];
...
function resolveTarget(
  ...
  let target: automerge.MaterializeValue = block;
...
function insertBlock(spans: automerge.Span[], patch: automerge.InsertPatch) {
  let idx = 0;
  let spanIdx = 0;
  const patchIndex = patch.path[patch.path.length - 1];
  if (typeof patchIndex !== 'number') {
    throw new Error(
      `Invalid patch path, expected a number got ", ${
        patch.path[patch.path.length - 1]
      }`,
    );
  }

  const newBlocks: automerge.Span[] = [];
...
export function findBlockAtCharIdx(
  spans: automerge.Span[],
  charIdx: number,
): { [key: string]: automerge.MaterializeValue } | null {
  let idx = 0;
...
function makeTextSpan({
  ...
  value: string;
  marks: automerge.MarkSet | undefined;
}): automerge.Span {
  const result: automerge.Span = { type: 'text', value };
  if (marks != null) {
    result.marks = marks;
  }
  return result;
