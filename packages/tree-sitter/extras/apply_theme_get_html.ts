 ...
 */
export const applyThemeGetHtml = (
  {
    themeRules,
    tree,
    string,
  ...
  const splicePoints = [
    ...new Set(Object.keys(spliceStarts).concat(Object.keys(spliceEnds))),
  ];
  ...
  for (const eachSplicePoint of splicePoints) {
    ...
    if (chunk.length != 0) {
      innerHtml.push(`<${tagForBaseElements}>${chunk}</${tagForBaseElements}>`);
    }
    // console.debug(`eachSplicePoint is:`, eachSplicePoint)
    // console.debug(`    spliceEnds[eachSplicePoint] is:`, spliceEnds[eachSplicePoint])
    // close first
    if (
      spliceEnds[eachSplicePoint] instanceof Array &&
      spliceEnds[eachSplicePoint].length > 0
    ) {
      for (
        const [styleAndClass, startIndex, eachPriority]
          of spliceEnds[eachSplicePoint]
      ) {
    ...
    // then open
    if (
      spliceStarts[eachSplicePoint] instanceof Array &&
      spliceStarts[eachSplicePoint].length > 0
    ) {
      // the .reverse() causes highest priority to be checked first
      for (
        const [styleAndClass, eachPriority] of spliceStarts[eachSplicePoint]
          .reverse()
      ) {
        ...
        if (eachPriority > activePriority) {
          ...
          if ((classNames || '').trim().length > 0) {
            innerHtml.push(
              `<${tagForStyledElements} style="${escapeHtml(style)}" class="${
                escapeHtml(classNames || '')
              }">`,
            );
          } else {
            innerHtml.push(
              `<${tagForStyledElements} style="${escapeHtml(style)}">`,
            );
          }
  ...
  innerHtml.push(
    `<${tagForBaseElements}>${
      escapeHtml(string.slice(prevSlicePoint))
    }</${tagForBaseElements}>`,
  );
