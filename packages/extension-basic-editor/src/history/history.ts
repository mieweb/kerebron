// Fork of https://code.haverbeke.berlin/prosemirror/prosemirror-history/
// @TODO: https://code.haverbeke.berlin/prosemirror/prosemirror-history/src/branch/main/test/test-history.ts

import { emptyRopeSequence, RopeSequence } from './RopeSequence.ts';
...
class Branch {
  ...
  addTransform(
    ...
    let oldItems = this.items,
      lastItem = !preserveItems && oldItems.length
  ...
  static empty = new Branch(emptyRopeSequence(), 0);
}
...
function histTransaction(
  ...
  let histOptions = (historyKey.get(state)!.spec as any).config as Required<
    HistoryOptions
  >;
  let pop = (redo ? history.undone : history.done).popEvent(
    state,
    preserveItems,
  );
  if (!pop) return null;
  ...
  let added = (redo ? history.done : history.undone).addTransform(
    pop.transform,
    ...
    state: {
      ...
      apply(tr, hist, state) {
        return applyTransaction(
          ...
          state,
          tr,
          config as Required<HistoryOptions>,
        );
      },
    },
    ...
    props: {
      handleDOMEvents: {
        beforeinput(view, e: Event) {
          ...
          let command = inputType == 'historyUndo'
            ? undo
