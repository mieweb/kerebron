...
type DocumentMarkdownTokenizerSpec = {
  ...
  open:
    | string
    | ((
      state: DocumentMarkdownTokenizerState,
      mark: Mark,
      parent: Node,
      index: number,
    ) => string);
...
const blankMark: DocumentMarkdownTokenizerSpec = {
  open: '',
  close: '',
  mixable: true,
};
...
export class DocumentMarkdownTokenizerOld {
  ...
  constructor(
    ...
    readonly marks: { [mark: string]: DocumentMarkdownTokenizerSpec },
    readonly options: {
  ...
  serialize(content: Node, options: {
    /// Whether to render lists in a tight assets. This can be overridden
    /// on a node level by specifying a tight attribute on the node.
    /// Defaults to false.
    tightLists?: boolean;
  } = {}) {
    options = Object.assign({}, this.options, options);
    let state = new DocumentMarkdownTokenizerState(
      this.nodes,
      this.marks,
      options,
    );
...
export class DocumentMarkdownTokenizerState {
  /// @internal
  delim: string = '';
  /// @internal
  out: string = '';
  ...
  render(node: Node, parent: Node, index: number) {
    if (this.nodes[node.type.name]) {
      ...
      if (this.options.strict !== false) {
        ...
        // );
        this.nodes['html'](this, node, parent, index);
    ...
    let progress = (node: Node | null, offset: number, index: number) => {
      ...
      if (node) {
        ...
        if (noEsc && node.isText) {
          this.text(
            this.markString(inner!, true, parent, index) + node.text +
  ...
  renderList(node: Node, delim: string, firstDelim: (index: number) => string) {
    ...
    let isTight = typeof node.attrs.tight != 'undefined'
      ? node.attrs.tight
  ...
  esc(str: string, startOfLine = false) {
    str = str.replace(
      /[`*\\~\[\]_]/g,
      (m, i) =>
        m == '_' && i > 0 && i + 1 < str.length && str[i - 1].match(/\w/) &&
          str[i + 1].match(/\w/)
          ? m
          : '\\' + m,
    );
