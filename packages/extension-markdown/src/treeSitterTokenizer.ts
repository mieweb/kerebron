        ...
        case 'shortcut_link':
          {
            ...
            // walkInline(node.children.filter(c => !!c));
        ...
        case 'image':
          {
            ...
            token.attrSet('alt', nodeText(image_description) || '');

            const destination = node.children
              .find((c) => c?.type === 'link_destination');
            token.attrSet('src', nodeText(destination) || '');

            const title = node.children
              .find((c) => c?.type === 'link_title');
            if (title) {
              token.attrSet('title', nodeText(title) || '');
            }
        ...
        case 'strong_emphasis':
          {
            ...
            const tokenName = nodeText(delimiter) === '_'
              ? 'underline'
              : 'strong';
            const tagName = nodeText(delimiter) === '_' ? 'u' : 'strong';
        ...
        case 'emphasis':
          {
            ...
            const tokenName = nodeText(delimiter) === '_' ? 'underline' : 'em';
            const tagName = nodeText(delimiter) === '_' ? 'u' : 'em';
            const openToken = new Token(
              tokenName + '_open',
              tagName,
              NESTING_OPENING,
      ...
      case 'pipe_table_cell':
        {
...

          {
            ...
            if (children.length > 0) {
              ...
              token.map = map;
              token.meta = 'noEscText';
              token.content = nodeText(node)?.replace(/\s+$/, '') ?? '';
