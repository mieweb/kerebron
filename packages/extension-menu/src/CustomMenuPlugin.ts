...
export class CustomMenuView {
  ...
  private pinnedDropdownStack: Array<{ title: string; tools: ToolItem[]; rootTool: ToolItem }> = [];
  ...
  private pinnedDropdownGoBack() {
    ...
    const triggerElement = this.toolbar.querySelector(`[data-tool-id="${this.pinnedDropdownStack[0].rootTool.id}"]`) as HTMLElement;
    if (triggerElement) {
      this.renderPinnedDropdown(triggerElement);
    }
  }

  private pinnedDropdownShowSubmenu(tool: ToolItem, triggerElement: HTMLElement) {
    const dropdown = tool.element as any;
    if (!dropdown.content) return;
    ...
    if (isSubmenu && currentSubmenu) {
      ...
      if (isMobile && mobileOverflowPinned.length > 0) {
        ...
        mobileOverflowPinned.forEach((tool) => {
          ...
          if (!tool.label || tool.label.trim() === '' || tool.label === 'Unknown Tool') {
            return;
          }
