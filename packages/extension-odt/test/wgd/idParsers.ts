...
export function replaceUrlsWithIds(url: string): string {
  url = url.replaceAll(
    'https://drive.google.com/open?id%3D',
    'https://drive.google.com/open?id=',
  );
  url = url.replaceAll('https://drive.google.com/open?id=', 'gdoc:');
  return url;
}
...
export function urlToFolderId(url: string): string | null {
  ...
  if (url.indexOf('https://drive.google.com/open?id=') > -1) {
    let id = url.substring(
      url.indexOf('https://drive.google.com/open?id=') +
        'https://drive.google.com/open?id='.length,
    );
    if (id.indexOf('&') > 0) {
      id = id.substring(0, id.indexOf('&'));
    }
