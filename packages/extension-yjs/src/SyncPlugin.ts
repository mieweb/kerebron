...
import { NormalizedPNodeContent } from 'y-prosemirror';
...
export class ProsemirrorBinding {
  type: Y.XmlFragment;
  mux: mutex;
  private _observeFunction: (events: any, transaction: any) => void;
  private prosemirrorView: EditorView | null;
  private doc: Y.Doc;
  beforeTransactionSelection: { anchor: any; head: any };
  afterAllTransactions: () => void;
  beforeAllTransactions: (arg0: Y.Doc) => void;

  constructor(
    private yXmlFragment: Y.XmlFragment,
    private mapping: ProsemirrorMapping = new Map(),
  ) {
    ...
    this.beforeAllTransactions = () => {
      if (
        this.beforeTransactionSelection === null && this.prosemirrorView != null
      ) {
   ...
   */
  initView(prosemirrorView: EditorView) {
...
export const createNodeFromYElement = (
  ...
  const createChildren = (type) => {
    if (type.constructor === Y.XmlElement) {
      ...
      if (
        nextytext != null && !nextytext._item.deleted &&
...
const matchNodeName = (yElement: Y.XmlElement, pNode: Node): boolean =>
