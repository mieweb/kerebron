...
const joinBackward = (): Command => (state, dispatch, view) => {
...
const joinTextblockBackward = (): Command => (state, dispatch, view) => {
  let $cursor = atBlockStart(state, view);
  if (!$cursor) return false;
  let $cut = findCutBefore($cursor);
...
const joinTextblockForward = (): Command => (state, dispatch, view) => {
...
function joinTextblocksAround(
  ...
  let step = replaceStep(state.doc, beforePos, afterPos, Slice.empty) as
    | ReplaceStep
    | null;
  if (
    !step || step.from != beforePos ||
...
const selectNodeBackward = (): Command => (state, dispatch, view) => {
...
const joinForward = (): Command => (state, dispatch, view) => {
  let $cursor = atBlockEnd(state, view);
  if (!$cursor) return false;
...
const selectNodeForward = (): Command => (state, dispatch, view) => {
  let { $head, empty } = state.selection, $cut: ResolvedPos | null = $head;
  if (!empty) return false;
  if ($head.parent.isTextblock) {
    if (
  ...
  if (dispatch) {
    dispatch(
      state.tr.setSelection(NodeSelection.create(state.doc, $cut!.pos))
        .scrollIntoView(),
    );
  }
  return true;
...
const joinUp = (): Command => (state, dispatch) => {
  let sel = state.selection, nodeSel = sel instanceof NodeSelection, point;
  if (nodeSel) {
    if (
      (sel as NodeSelection).node.isTextblock || !canJoin(state.doc, sel.from)
    ) return false;
    point = sel.from;
  } else {
...
const joinDown = (): Command => (state, dispatch) => {
...
const splitBlockAs = (
  ...
  return (state, dispatch) => {
    ...
    if (!atEnd && atStart && $from.node(splitDepth).type != deflt) {
      ...
      if (
        deflt &&
        $from.node(splitDepth - 1).canReplaceWith(
          $first.index(),
          $first.index() + 1,
          deflt,
        )
...
const splitBlock = (): Command => splitBlockAs();
...
const selectParentNode = (): Command => (state, dispatch) => {
...
function deleteBarrier(
  ...
  if (
    ...
    if (dispatch) {
      ...
      if (
        $joinAt.nodeAfter && $joinAt.nodeAfter.type == before.type &&
  ...
  if (
    canDelAfter && textblockAt(after, 'start', true) &&
    textblockAt(before, 'end')
  ) {
    let at = before, wrap = [];
    for (;;) {
...
function removeInlineAtoms(
  ...
  for (let i = 0; i < ranges.length; i++) {
    ...
    $from.doc.nodesBetween($from.pos, $to.pos, (node, pos) => {
      if (
        node.isAtom && node.content.size && node.isInline && pos >= $from.pos &&
        pos + node.nodeSize <= $to.pos
      ) {
        if (pos + 1 > $from.pos) {
