  ...
  (async () => {
    try {
      ...
      return Promise.all(
        ...
        const locs = resolved.filter((l) => !!l) as ReferenceLocation[];
        if (locs.length) {
      ...
      plugin.reportError('Finding references failed', err);
    } finally {
...
function createReferencePanel(
  ...
  return (view) => {
    ...
    for (let { file, range } of locs) {
      ...
      entry.appendChild(document.createElement('strong')).textContent = line
        .text.slice(from - line.from, to - line.from);
    ...
    async function showReference(index: number) {
      ...
      if (pos) {
        ...
        ui.select({
          anchor: pos,
          scrollIntoView: true,
          userEvent: 'select.showReference',
        });
      }
    }
